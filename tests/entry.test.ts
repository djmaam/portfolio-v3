...
test('markTransform starts collapsed at the center of the viewport', () => {
  ...
  expect(frame.scale).toBeCloseTo(Math.min(VIEW.w, VIEW.h) * 0.85 * 0.25, 12)
  expect(frame.alpha).toBeCloseTo(0.35, 12)
})

test('the expansion fills the shorter side of any viewport', () => {
  // The mark owns the screen while it is out there, on a phone as much as on a desktop:
  // the box it is fitted into is the same fraction of the shorter side either way.
  ...
  expect(narrow / wide).toBeCloseTo(phone.w / Math.min(VIEW.w, VIEW.h), 10)
  expect(wide).toBeGreaterThan(Math.min(VIEW.w, VIEW.h) * 0.85 * 0.99)
  expect(narrow).toBeGreaterThan(phone.w * 0.85 * 0.99)
  // And never wider than the viewport it is drawn on.
  expect(narrow).toBeLessThan(phone.w)
})
