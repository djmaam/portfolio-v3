...
- **Logos for AWS and LinkedIn.** simple-icons dropped both marks over brand policy, so
  shields.io serves those badges without an icon. They are written logo-less on purpose.
- **`github-readme-stats` cards.** The public instance answers 503 often enough that a
  broken image on the profile is the likely state, not the exception. Self-host it if the
  cards are worth it.
