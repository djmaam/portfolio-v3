...
**All six previews are static screenshots** (`public/previews/<host>.jpg`), regenerated by
`scripts/capture-previews.ts`.

Two of them always were: `creativamedialab.com` answers `X-Frame-Options: DENY` and
`telecentro.com.ar` sits behind a WAF (checked 2026-09-05). The other four moved from live
iframes to screenshots in issue 17, for the Lighthouse budget: four cross-origin embeds
pulled 40-50 third-party requests into the page — Tag Manager, Wix, Cloudflare Insights —
which cost `/en` Performance 0.82 and Best Practices 0.79, and made the budget depend on
CDN warmth rather than on our own code. With static images both pages score 1.00 / 0.96 /
1.00 / 1.00.

The mode stays declared per host in `src/lib/projects.ts`, never by position in
`content.json`, and a unit test asserts every project has a mode and an image on disk.

Images are JPEG at 960x660 (the 16:11 of the card box, ~2.7x its painted width): 723 KB
across the six, against 2.5 MB as PNG. `loading="lazy"`, `alt=""`, `pointer-events: none`.
